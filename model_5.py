...
path_checkpoints_5 = "model_checkpoints/q3a/model_5"
if not os.path.exists(path_checkpoints_5):
    os.makedirs(path_checkpoints_5)
    
# Make model backup directory
path_backup_5 = "model_backup/q3a/model_5"
if not os.path.exists(path_backup_5):
    os.makedirs(path_backup_5)

# Delete log file if it exists
if os.path.exists('training_logs/model_5.csv'):
    os.remove('training_logs/model_5.csv')

# clear model checkpoints directory
for file in os.listdir(path_checkpoints_5):
    if os.path.isfile(file):
        of.remove(path_checkpoints_5+'/'+file)

# Define callback for Backing up and restoring training state in case of interruption
backup_restore_5 = tf.keras.callbacks.BackupAndRestore(path_backup_5)

# Define callback for logging training results
csv_logging_5 = tf.keras.callbacks.CSVLogger('training_logs/model_5.csv',append=True) #Setting append to true in case of interruptions

# Define callback for saving the best model
model_checkpoints_5 = tf.keras.callbacks.ModelCheckpoint(
    filepath=path_checkpoints_5+"/epoch_{epoch}.weights.h5",
...
### EXACT SAME ARCHITECTURE AS MODEL 2
...
# Compile and fit the model with increased gamma and RVQ Layers
rvqvae_trainer5 = RVQVAETrainer2(train_var,shape=(128,128,2),latent_dim=16,num_embeddings=128,rvq_layers=10,gamma=0.999)
rvqvae_trainer5.compile(optimizer=optimizer,loss='loss')
model_5_hist = rvqvae_trainer5.fit(
    train_ds,
    validation_data = val_ds,
    epochs=40,
    callbacks=[backup_restore_5,csv_logging_5,model_checkpoints_5]
)